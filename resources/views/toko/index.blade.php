        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-lg py-4 space-y-4">
                <div class="flex flex-row justify-between px-4">
                    <div></div>
                    <x-primary-button-link href="{{ route('toko.create') }}">
                        {{ __('Add toko') }}
                        ...
                        <thead>
                            <tr>
                                <th class="bg-gray-50 px-6 py-3 text-left">
                                    <span class="text-xs font-medium uppercase leading-4 tracking-wider text-gray-500">Nama toko</span>
                                </th>
                                <th class="bg-gray-50 px-6 py-3 text-left">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 divide-solid">
                            @foreach ($tokos as $index => $toko)
                                <tr class="bg-white">
                                    ...
                                    <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                        {{ date('d M Y', strtotime($toko->created_at)) }}
                                    </td>

                                    <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                        <x-edit-button href="{{ route('toko.edit', $toko) }}" />
                                        <form action="{{ route('toko.destroy', $toko) }}" method="POST" onsubmit="return confirm('Are you sure?')" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="px-4">
                    {{ $tokos->links('components.pagination') }}
                </div>
            </div>
        </div>
    </div>
